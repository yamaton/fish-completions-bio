...
complete -c gffread -l "ids" -d "discard records/transcripts if their IDs are not listed in <IDs.lst>" -x
complete -c gffread -l "nids" -d "discard records/transcripts if their IDs are listed in <IDs.lst>" -x
complete -c gffread -s "i" -d "discard transcripts having an intron larger than <maxintron>" -x
complete -c gffread -s "l" -d "discard transcripts shorter than <minlen> bases" -x
complete -c gffread -s "r" -d "only show transcripts overlapping coordinate range <start>..<end> (on chromosome/contig <chr>, strand <strand> if provided)" -x
complete -c gffread -s "R" -d "for -r option, discard all transcripts that are not fully contained within the given range"
complete -c gffread -l "jmatch" -d "only output transcripts matching the given junction" -x
...
complete -c gffread -s "s" -d "a tab-delimited file providing this info   for each of the mapped sequences: <seq-name> <seq-length> <seq-description> (useful for -A option with mRNA/EST/protein mappings)" -r
complete -c gffread -l "sort-alpha" -d "chromosomes (reference sequences) are sorted alphabetically"
complete -c gffread -l "sort-by" -d "sort the reference sequences by the order in which their names are given in the <refseq.lst> file" -r
complete -c gffread -s "F" -d "keep all GFF attributes (for non-exon features)"
...
complete -c gffread -l "attrs" -d "only output the GTF/GFF attributes listed in <attr-list> which is a comma delimited list of attribute names to" -x
...
complete -c gffread -s "M" -l "merge" -d "cluster the input transcripts into loci, discarding \"redundant\" transcripts (those with the same exact introns and fully contained or equal boundaries)"
...
complete -c gffread -s "K" -d "for -M option: also discard as redundant the shorter, fully contained"
...
complete -c gffread -s "g" -d "full path to a multi-fasta file with the genomic sequences for all input mappings, OR a directory with single-fasta files (one per genomic sequence, with file names matching sequence names)" -r
complete -c gffread -s "j" -d "output the junctions and the corresponding transcripts"
complete -c gffread -s "w" -d "write a fasta file with spliced exons for each transcript" -r
...
complete -c gffread -l "w-nocds" -d "for -w, disable the output of CDS info in the FASTA file"
complete -c gffread -s "x" -d "write a fasta file with spliced CDS for each GFF transcript" -r
complete -c gffread -s "y" -d "write a protein fasta file with the translation of CDS for each record" -r
complete -c gffread -s "W" -d "for -w, -x and -y options, write in the FASTA defline all the exon coordinates projected onto the spliced sequence;"
complete -c gffread -s "S" -d "for -y option, use '*' instead of '.' as stop codon translation"
complete -c gffread -s "L" -d "Ensembl GTF to GFF3 conversion, adds version to IDs"
complete -c gffread -s "m" -d "<chr_replace> is a name mapping table for converting reference sequence names, having this 2-column format: <original_ref_ID> <new_ref_ID>"
complete -c gffread -s "t" -d "use <trackname> in the 2nd column of each GFF/GTF output line" -x
complete -c gffread -s "o" -d "write the output records into <outfile> instead of stdout" -r
