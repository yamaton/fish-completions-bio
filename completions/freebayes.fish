...
complete -c freebayes -s "h" -l "help" -d "For a complete description of options."
complete -c freebayes -l "haplotype-length" -d "# naive variant calling: simply annotate observation counts of SNPs and indels" -x
...
complete -c freebayes -l "populations" -d "Each line of FILE should list a sample and a population which it is part of." -r
complete -c freebayes -s "A" -l "cnv-map" -d "Read a copy number map from the BED file FILE, which has either a sample-level ploidy:" -r
...
complete -c freebayes -s "&" -l "gvcf-dont-use-chunk" -d "When writing the gVCF output emit a record for all bases if set to \"true\" , will also route an int to --gvcf-chunk similar to --output-mode EMIT_ALL_SITES from GATK" -x
complete -c freebayes -s "@" -l "variant-input" -d "Use variants reported in VCF file as input to the algorithm." -r
