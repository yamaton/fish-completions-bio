...
complete -c htseq-count -s f -l format -d 'Type of <alignment_file> data.' -x
complete -c htseq-count -s r -l order -d '\'pos\' or \'name\'.' -x
complete -c htseq-count -l max-reads-in-buffer -d 'When <alignment_file> is paired end sorted by position, allow only so many reads to stay in memory until the mates are found (raising this number will use more memory).' -x
complete -c htseq-count -s s -l stranded -d 'Whether the data is from a strand-specific assay.' -x
complete -c htseq-count -s a -l minaqual -d 'Skip all reads with MAPQ alignment quality lower than the given minimum value (default: 10).' -x
complete -c htseq-count -s t -l type -d 'Feature type (3rd column in GTF file) to be used, all features of other type are ignored (default, suitable for Ensembl GTF files: exon)' -x
complete -c htseq-count -s i -l idattr -d 'GTF attribute to be used as feature ID (default, suitable for Ensembl GTF files: gene_id).' -x
complete -c htseq-count -l additional-attr -d 'Additional feature attributes (default: none, suitable for Ensembl GTF files: gene_name).' -x
...
complete -c htseq-count -s o -l samout -d 'Write out all SAM alignment records into SAM/BAM files (one per input file needed), annotating each line with its feature assignment (as an optional field with tag \'XF\').' -x
...
complete -c htseq-count -s n -l nprocesses -d 'Number of parallel CPU processes to use (default: 1).' -x
