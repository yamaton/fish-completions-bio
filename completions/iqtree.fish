...
complete -c iqtree -l "alisim" -d "Activate AliSim and specify the output alignment filename" -r
complete -c iqtree -s "t" -d "Set the input tree file name" -r
complete -c iqtree -l "length" -d "Set the length of the root sequence" -x
complete -c iqtree -l "num-alignments" -d "Set the number of output datasets" -x
complete -c iqtree -l "seqtype" -d "BIN, DNA, AA, CODON, MORPH{NUM_STATES} (default: auto-detect) For morphological data, 0<NUM_STATES<=32" -x
complete -c iqtree -l "m" -d "Specify the evolutionary model." -x
complete -c iqtree -l "mdef" -d "Name of a NEXUS model file to define new models (see Manual)" -r
complete -c iqtree -l "fundi" -d "Specify a list of taxa, and Rho (Fundi weight) for FunDi model" -x
complete -c iqtree -l "indel" -d "Set the insertion and deletion rate of the indel model, relative to the substitution rate" -x
complete -c iqtree -l "indel-size" -d "Set the insertion and deletion size distributions" -x
complete -c iqtree -l "sub-level-mixture" -d "Enable the feature to simulate substitution-level mixture model"
complete -c iqtree -l "no-unaligned" -d "Disable outputing a file of unaligned sequences when using indel models"
complete -c iqtree -l "root-seq" -d "Specify the root sequence from an alignment" -r
complete -c iqtree -s "s" -d "Specify the input sequence alignment" -r
complete -c iqtree -l "no-copy-gaps" -d "Disable copying gaps from input alignment (default: false)"
complete -c iqtree -l "site-freq" -d "Specify the option (MEAN (default), or SAMPLING, or MODEL) to mimic the site-frequencies for mixture models from the input alignment (see Manual)" -x
...
complete -c iqtree -l "single-output" -d "Output all alignments into a single file"
complete -c iqtree -l "write-all" -d "Enable outputting internal sequences"
complete -c iqtree -l "seed" -d "Random seed number (default: CPU clock) Be careful to make the AliSim reproducible, users should specify the seed number" -x
complete -c iqtree -o "gz" -d "Enable output compression but taking longer running time"
complete -c iqtree -o "af" -d "Set the output format (default: phylip)" -x
