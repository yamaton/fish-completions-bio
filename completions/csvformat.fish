...
complete -c csvformat -s u -l quoting -d 'Quoting style used in the input CSV file.' -x
...
complete -c csvformat -s z -l maxfieldsize -d 'Maximum length of a single field in the input CSV file.' -x
...
complete -c csvformat -s Q -l out-quotechar -d 'Character used to quote strings in the output CSV file.' -x
...
complete -c csvformat -s P -l out-escapechar -d 'Character used to escape the delimiter in the output CSV file if --quoting 3 ("Quote None") is specified and to escape the QUOTECHAR if --no-doublequote is specified.' -x
complete -c csvformat -s M -l out-lineterminator -d 'Character used to terminate lines in the output CSV file.' -x
