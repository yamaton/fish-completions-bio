...
complete -c fastp -l unpaired1 -d 'for PE input, if read1 passed QC but read2 not, it will be written to unpaired1.'
complete -c fastp -l unpaired2 -d 'for PE input, if read2 passed QC but read1 not, it will be written to unpaired2.'
complete -c fastp -l failed_out -d 'specify the file to store reads that cannot pass the filters.'
...
complete -c fastp -l reads_to_process -d 'specify how many reads/pairs to be processed.'
...
complete -c fastp -s a -l adapter_sequence -d 'the adapter for read1.'
complete -c fastp -l adapter_sequence_r2 -d 'the adapter for read2 (PE data only).'
...
complete -c fastp -s b -l max_len1 -d 'if read1 is longer than max_len1, then trim read1 at its tail to make it as long as max_len1.'
complete -c fastp -s F -l trim_front2 -d 'trimming how many bases in front for read2.'
complete -c fastp -s T -l trim_tail2 -d 'trimming how many bases in tail for read2.'
complete -c fastp -s B -l max_len2 -d 'if read2 is longer than max_len2, then trim read2 at its tail to make it as long as max_len2.'
...
complete -c fastp -l poly_g_min_len -d 'the minimum length to detect polyG in the read tail.'
...
complete -c fastp -l poly_x_min_len -d 'the minimum length to detect polyX in the read tail.'
...
complete -c fastp -s W -l cut_window_size -d 'the window size option shared by cut_front, cut_tail or cut_sliding.'
complete -c fastp -s M -l cut_mean_quality -d 'the mean quality requirement option shared by cut_front, cut_tail or cut_sliding.'
...
complete -c fastp -s q -l qualified_quality_phred -d 'the quality value that a base is qualified.'
complete -c fastp -s u -l unqualified_percent_limit -d 'how many percents of bases are allowed to be unqualified (0~100).' -x
complete -c fastp -s n -l n_base_limit -d 'if one read\'s number of N base is >n_base_limit, then this read/pair is discarded.'
complete -c fastp -s e -l average_qual -d 'if one read\'s average quality score <avg_qual, then this read/pair is discarded.'

complete -c fastp -s L -l disable_length_filtering -d 'length filtering is enabled by default.'
complete -c fastp -s l -l length_required -d 'reads shorter than length_required will be discarded, default is 15.'
complete -c fastp -l length_limit -d 'reads longer than length_limit will be discarded, default 0 means no limitation.'
...
complete -c fastp -l filter_by_index_threshold -d 'the allowed difference of index barcode for index filtering, default 0 means completely identical.'

complete -c fastp -s c -l correction -d 'enable base correction in overlapped regions (only for PE data), default is disabled'
complete -c fastp -l overlap_len_require -d 'the minimum length to detect overlapped region of PE reads.'
complete -c fastp -l overlap_diff_limit -d 'the maximum number of mismatched bases to detect overlapped region of PE reads.'
complete -c fastp -l overlap_diff_percent_limit -d 'the maximum percentage of mismatched bases to detect overlapped region of PE reads.'

complete -c fastp -s U -l umi -d 'enable unique molecular identifier (UMI) preprocessing'
complete -c fastp -l umi_loc -d 'specify the location of UMI, can be (index1/index2/read1/read2/per_index/per_read, default is none (string [=])' -r
complete -c fastp -l umi_len -d 'if the UMI is in read1/read2, its length should be provided (int [=0])' -x
complete -c fastp -l umi_prefix -d 'if specified, an underline will be used to connect prefix and UMI (i.e. prefix=UMI, UMI=AATTCG, final=UMI_AATTCG).'
...
complete -c fastp -s P -l overrepresentation_sampling -d 'one in (--overrepresentation_sampling) reads will be computed for overrepresentation analysis (1~10000), smaller is slower, default is 20.'
...
complete -c fastp -s R -l report_title -d 'should be quoted with \' or ", default is "fastp report" (string [=fastp report])' -r
