...
complete -c fastp -l "overlapped_out" -d "for each read pair, output the overlapped region if it has no any mismatched base."
...
complete -c fastp -s "D" -l "dedup" -d "enable deduplication to drop the duplicated reads/pairs"
complete -c fastp -l "dup_calc_accuracy" -d "accuracy level to calculate duplication (1~6), higher level uses more memory (1G, 2G, 4G, 8G, 16G, 24G)."
complete -c fastp -l "dont_eval_duplication" -d "don't evaluate duplication rate to save time and use less memory."
...
complete -c fastp -s "w" -l "thread" -d "worker thread number, default is 3 (int [=3])" -x
