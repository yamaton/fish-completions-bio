...
complete -c multiqc -s t -l template -d 'Report template to use.'-x -a "default default_dev gathered geo sections simple"
complete -c multiqc -l tag -d 'Use only modules which tagged with this keyword, eg. RNA' -x
...
complete -c multiqc -l sample-names -d 'File containing alternative sample names' -x
complete -c multiqc -l sample-filters -d 'File containing show/hide patterns for the report' -x
...
complete -c multiqc -s k -l data-format -d 'Output parsed data in a different format.' -x -a "tsv json yaml"
...
complete -c multiqc -l lint -d 'Use strict linting (validation) to help code'
...
complete -c multiqc -s c -l config -d 'Specific config file to load, after those in' -x
