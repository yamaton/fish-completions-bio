...
complete -c unicycler_check -l "sam" -d "Input SAM file of alignments (if this file doesn't exist, the alignment will be performed with results saved to this file - you can use the aligner arguments with this script)" -r
complete -c unicycler_check -l "ref" -d "FASTA file containing one or more reference sequences" -r
complete -c unicycler_check -l "reads" -d "FASTQ file of long reads" -r
...
complete -c unicycler_check -l "window_tables" -d "Path and/or prefix for table files summarising reference errors for reference windows (default: do not save window tables)" -r
complete -c unicycler_check -l "base_tables" -d "Path and/or prefix for table files summarising reference errors at each base (default: do not save base tables)" -r
complete -c unicycler_check -l "html" -d "Path for HTML report (default: do not save HTML report)" -r
