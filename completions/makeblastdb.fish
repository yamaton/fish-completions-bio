...
complete -c makeblastdb -o input_type -d 'Type of the data specified in input_file (Default = `fasta\')' -r
complete -c makeblastdb -o title -d 'Title for BLAST database (Default = input file name provided to -in argument)' -r
...
complete -c makeblastdb -o mask_data -d 'Comma-separated list of input files containing masking data as produced by NCBI masking applications (e.g. dustmasker, segmasker, windowmasker)' -r
...
complete -c makeblastdb -o gi_mask_name -d 'Comma-separated list of masking data output files.' -r
complete -c makeblastdb -o out -d 'Name of BLAST database to be created  Default = input file name provided to -in argumentRequired if multiple file(s)/database(s) are provided as input' -r
complete -c makeblastdb -o blastdb_version -d 'Version of BLAST database to be created (Default = `5\')' -x
complete -c makeblastdb -o max_file_sz -d 'Maximum file size for BLAST database files (Default = `1GB\')' -r
