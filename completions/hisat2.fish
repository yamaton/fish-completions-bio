...
complete -c hisat2 -s "x" -d "Index filename prefix (minus trailing .X.ht2)." -r
complete -c hisat2 -s "1" -d "Files with #1 mates, paired with files in <m2>." -r
complete -c hisat2 -s "2" -d "Files with #2 mates, paired with files in <m1>." -r
complete -c hisat2 -s "U" -d "Files with unpaired reads." -r
complete -c hisat2 -s "S" -d "File for SAM output (default: stdout)" -r
