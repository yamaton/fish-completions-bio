...
complete -c csvsql -s u -l quoting -d 'Quoting style used in the input CSV file.' -x
...
complete -c csvsql -s z -l maxfieldsize -d 'Maximum length of a single field in the input CSV file.' -x
...
complete -c csvsql -l db -d 'If present, a SQLAlchemy connection string to use to directly execute generated SQL on a database.' -x
complete -c csvsql -l query -d 'Execute one or more SQL queries delimited by ";" and output the result of the last query as CSV.' -x
...
complete -c csvsql -l tables -d 'A comma-separated list of names of tables to be created.' -x
...
complete -c csvsql -s y -l snifflimit -d 'Limit CSV dialect sniffing to the specified number of bytes.' -x
